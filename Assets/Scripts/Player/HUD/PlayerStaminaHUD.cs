...
namespace proscryption
{
    public class PlayerStaminaHUD : MonoBehaviour
    {
        ...
        {
            staminaFrame.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal,
                _frameStartWidth + (staminaUpgrades * _frameStepValue));
            _maxRightMask = fillBarRect.rect.width - mask.padding.x - mask.padding.z > _maskLimit
                ? fillBarRect.rect.width - mask.padding.x - mask.padding.z
                : _maskLimit;
            maskRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal,
                _initialMaskWidth + (staminaUpgrades * _frameStepValue / 3.15f));
            maskFormatRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal,
                _initialMaskWidth + (staminaUpgrades * _frameStepValue));
